...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
        ...
        String cleanStr;
        Character[] charArray;
        ArrayListStack<Character> word;
        if(s == null) throw new IllegalArgumentException();
        cleanStr = s.replaceAll("\\s", "").toLowerCase();
        charArray = new Character[cleanStr.length()];
        for(i = 0; i<charArray.length; ++i)
            charArray[i] = Character.valueOf(cleanStr.charAt(i));
        word = new ArrayListStack<>(charArray);
